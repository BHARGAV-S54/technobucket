...
// Aurora configuration - Techno theme colors (cyan, blue, purple)
const config = {
  speed: 0.7,
  scale: 1.8,
  brightness: 1.2,
...
function enableFallback(container) {
  if (!container) return;
...
function initAurora(container) {
  ...
  try {
    renderer = new Renderer({
      ...
      // Cap DPR to avoid high fill-rate on Retina screens (major source of lag).
      dpr: Math.min(window.devicePixelRatio || 1, 1.3)
    });
  } catch (e) {
    enableFallback(container);
    return;
  }
  ...
  if (typeof IntersectionObserver !== 'undefined') {
    ...
    inView = true; // fallback: always render if observer not supported
  }
